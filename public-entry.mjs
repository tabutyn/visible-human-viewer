// Static release entry: no dataset, renderer, or GPU initialization before consent.
...
async function start(){
  ...
  try{
    ...
    if(!response.ok)throw new Error("Unable to load the pinned release configuration.");
    const config=await response.json();
    if(config.readOnly!==true||!config.releaseManifestUrl)throw new Error("The public release configuration is invalid.");
    window.visibleHumanConfig=Object.freeze(config);document.body.dataset.readOnly="true";document.body.dataset.activeTab="medical";history.replaceState(null,"",`${location.pathname}${location.search}#medical`);
    ...
    document.querySelector(".viewer-shell").hidden=false;gate.hidden=true;document.querySelector("#release-credit").hidden=false;window.dispatchEvent(new Event("resize"));
...
form.addEventListener("submit",event=>{event.preventDefault();if(!age.checked||!content.checked)return;try{sessionStorage.setItem(key,JSON.stringify({version:1,acceptedAt:Date.now()}));}catch{}start();});
document.querySelector("#leave-viewer").addEventListener("click",()=>{forget();age.checked=false;content.checked=false;enter.disabled=true;status.textContent="No cadaver images were loaded. You can close this tab.";});
document.querySelector("#hide-imagery").addEventListener("click",()=>{document.querySelector(".viewer-shell").hidden=true;gate.hidden=false;forget();location.reload();});
enter.disabled=!(age.checked&&content.checked);form.dataset.ready="true";if(accepted())start();
